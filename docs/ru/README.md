...
Action автоматически предоставляет доступ к переменным контекста GitHub Actions без ручной настройки в `template_vars`. Все переменные доступны в формате `{{variableName}}` в ваших сообщениях.
...
```yaml
- name: Отправить уведомление с авто-контекстом
  ...
  with:
    ...
    inline_keyboard: |
      [
        [
          {"text": "🏠 Репозиторий", "url": "{{serverUrl}}/{{repository}}"},
          {"text": "🔄 Этот запуск", "url": "{{serverUrl}}/{{repository}}/actions/runs/{{runId}}"}
        ]
      ]
```
