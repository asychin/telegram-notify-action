...
[![Версия](https://img.shields.io/badge/version-3.1.0-blue.svg)](#)
...
**Вариант 1: Скачать правила ИИ в ваш репозиторий**
...
1. **📋 Задавать Комплексные Вопросы**
   ...
   - Какие типы уведомлений вам нужны? (сборка/деплой/тесты/события репозитория/и др.)
...
## 🔐 Интеграция с GitHub API и права доступа

### Когда нужны права доступа

Если ваш workflow использует вызовы GitHub API (мониторинг, статистика, данные репозитория), вы **ОБЯЗАТЕЛЬНО** должны добавить блок `permissions:` в свою задачу:
...
jobs:
  your-job:
    ...
    permissions:
      contents: read # ✅ Обязательно - Базовый доступ к репозиторию
      actions: read # ✅ Добавить при использовании API workflow runs
      issues: read # ✅ Добавить при использовании API issues
      pull-requests: read # ✅ Добавить при использовании API PRs

    steps:
      - name: Получить данные workflow
        run: |
          # Используйте github.token для вызовов API
          curl -H "Authorization: token ${{ github.token }}" \
            "https://api.github.com/repos/${{ github.repository }}/actions/runs"
```

### Необходимые права по случаям использования

| **Случай использования**   | **Необходимые права**                                                             | **Пример**                               |
| -------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------- |
| **Базовые уведомления**    | `contents: read`                                                                  | Простые сообщения об успехе/неудаче      |
| **Мониторинг репозитория** | `contents: read`<br/>`actions: read`                                              | Статус workflow, статистика сборок       |
| **Отслеживание Issues**    | `contents: read`<br/>`issues: read`                                               | Уведомления о создании/закрытии issue    |
| **Мониторинг PR**          | `contents: read`<br/>`pull-requests: read`                                        | Статус PR, уведомления о ревью           |
| **Комплексный мониторинг** | `contents: read`<br/>`actions: read`<br/>`issues: read`<br/>`pull-requests: read` | Полный мониторинг активности репозитория |

### Частые проблемы и решения

**Проблема:** Ошибка `HTTP 403 Forbidden` от GitHub API  
**Решение:** Добавьте недостающие права в вашу задачу:

```yaml
permissions:
  contents: read
  actions: read # Добавьте это для API workflow runs
```

**Проблема:** Ошибка `HTTP 404 Not Found` от Actions API  
**Решение:** Проверьте настройки репозитория:

- Репозиторий → Settings → Actions → General
- Убедитесь что "Actions permissions" разрешает запуск workflow
- Проверьте что "Workflow permissions" имеет доступ на чтение

### Лучшие практики аутентификации

```yaml
- name: Вызов GitHub API
  run: |
    # ✅ РЕКОМЕНДУЕТСЯ: Используйте github.token (автоматические права)
    curl -H "Authorization: token ${{ github.token }}" \
      "https://api.github.com/repos/${{ github.repository }}/actions/runs"

    # ⚠️ АЛЬТЕРНАТИВА: Используйте пользовательский токен (требует настройки)
    curl -H "Authorization: token ${{ secrets.GITHUB_TOKEN }}" \
      "https://api.github.com/repos/${{ github.repository }}/actions/runs"
```

**Когда использовать `github.token`:** Стандартный мониторинг репозитория (рекомендуется)  
**Когда использовать `secrets.GITHUB_TOKEN`:** Доступ между репозиториями или продвинутые операции
