...
| Параметр        | Описание                  | По умолчанию | Пример                                                             |
| --------------- | ------------------------- | ------------ | ------------------------------------------------------------------ |
| `template`      | Имя шаблона               | -            | `success`, `error`, `warning`, `info`, `deploy`, `deploy_detailed`, `test`, `pr_detailed`, `release` |
...
| `deploy_detailed`| **🆕 Детальное развертывание**   | Развертывания с полной статистикой PR    |
| `test`           | Уведомление о результатах тестов | Результаты наборов тестов                |
| `pr_detailed`    | **🆕 Детальный Pull Request**    | PR с подробной статистикой изменений     |
| `release`        | Уведомление о релизе             | Новые релизы, обновления версий          |

#### 🆕 Новые детальные шаблоны

**`deploy_detailed`** - Расширенный шаблон для развертываний, включающий:
- 📊 Статистику изменений (файлы, коммиты, добавления/удаления)
- 🌿 Сравнение веток (откуда → куда)
- 👤 Информацию об авторе и дате создания
- 📝 Описание Pull Request

**`pr_detailed`** - Подробный шаблон для Pull Request, включающий:
- 📈 Полную статистику изменений
- 💬 Количество комментариев и ревью
- 🏷️ Метки и назначенных пользователей
- 🔗 Прямую ссылку на PR
...
- `branchName` - **🆕** Правильное имя ветки (использует `headBranch` для PR)
- `branchComparison` - **🆕** Сравнение веток (`headBranch → baseBranch`)
- `isDraft` - Является ли черновиком
- `mergeable` - Можно ли слить
- `labels` - Список меток через запятую
- `assignees` - Список исполнителей через запятую
- `prCommits` - **🆕** Количество коммитов в PR
- `prAdditions` - **🆕** Количество добавленных строк
- `prDeletions` - **🆕** Количество удаленных строк
- `prChangedFiles` - **🆕** Количество измененных файлов
- `changesStats` - **🆕** Сводка изменений (`+540 ➕ -77 ➖`)
- `prComments` - **🆕** Количество комментариев
- `prReviewComments` - **🆕** Количество комментариев к коду
- `prCreatedAt` - **🆕** Дата создания PR
- `prUpdatedAt` - **🆕** Дата последнего обновления PR
...
### 🆕 Детальные уведомления о Pull Request

```yaml
name: Детальные PR уведомления
...
jobs:
  notify:
    runs-on: ubuntu-latest
    steps:
      - name: Детальное уведомление о PR
        uses: asychin/telegram-notify-action@v3
        with:
          telegram_token: ${{ secrets.TELEGRAM_BOT_TOKEN }}
          chat_id: ${{ secrets.TELEGRAM_CHAT_ID }}
          template: deploy_detailed  # Используем детальный шаблон
          language: ru
          template_vars: |
            {
              "deployStatus": "${{ github.event.pull_request.state }}",
              "customMessage": "Pull Request готов к ревью! 🔍"
            }
          inline_keyboard: |
            [
              [
                {"text": "👀 Посмотреть PR", "url": "${{ github.event.pull_request.html_url }}"},
                {"text": "📝 Ревью", "url": "${{ github.event.pull_request.html_url }}/files"}
              ],
              [
...
- 📊 Полную статистику изменений (файлы, коммиты, +/-)
- 🌿 Сравнение веток (feature-branch → main)
- 👤 Информацию об авторе и дате создания
- 📝 Описание Pull Request
- 💬 Количество комментариев и ревью
