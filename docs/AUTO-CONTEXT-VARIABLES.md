...
| `refProtected` | Защищена ли ветка | `true`, `false` |
| `sha` | SHA коммита | `"abc123..."` |
| `actor` | Пользователь, запустивший workflow | `"username"` |
| `triggeredBy` | Инициатор запуска (при повторном запуске) | `"username"` |
| `actorId` | ID пользователя | `"12345"` |
| `workflow` | Имя workflow | `"CI/CD Pipeline"` |
| `workflowRef` | Ссылка на workflow файл | `"owner/repo/.github/workflows/ci.yml@refs/heads/main"` |
| `workflowSha` | SHA workflow файла | `"def456..."` |
| `job` | Имя текущей задачи | `"build"` |
| `runId` | ID запуска workflow | `"123456789"` |
| `runNumber` | Номер запуска | `"42"` |
| `runAttempt` | Номер попытки запуска | `"1"` |
...
| `workspace` | Рабочая директория | `"/home/runner/work/repo/repo"` |
| `eventPath` | Путь к файлу события | `"/home/runner/work/_temp/_github_workflow/event.json"` |

### Окружение (Environment)

| Переменная | Описание | Пример |
|------------|----------|--------|
| `actionsEnvironment` | Имя окружения GitHub Actions | `"production"`, `"staging"` |
| `actionsEnvironmentId` | ID окружения | `"123456"` |
| `actionsEnvironmentUrl` | URL окружения | `"https://github.com/owner/repo/deployments/activity_log?environment=production"` |

### Дополнительные переменные

| Переменная | Описание | Пример |
|------------|----------|--------|
| `triggeredAt` | Время запуска workflow | `"2024-01-15T10:30:00Z"` |
| `jobUrl` | URL текущей задачи | `"https://github.com/owner/repo/actions/runs/123456/job/789"` |
| `stepUrl` | URL текущего шага | `"https://github.com/owner/repo/actions/runs/123456/job/789#step:3:1"` |

### Runner переменные

| Переменная | Описание | Пример |
|------------|----------|--------|
| `runnerOs` | Операционная система | `"Linux"`, `"Windows"`, `"macOS"` |
| `runnerArch` | Архитектура процессора | `"X64"`, `"ARM64"` |
| `runnerName` | Имя runner'а | `"GitHub Actions 2"` |
| `runnerEnvironment` | Тип runner'а | `"github-hosted"`, `"self-hosted"` |
| `runnerTemp` | Временная директория | `"/home/runner/work/_temp"` |
| `runnerToolCache` | Кэш инструментов | `"/opt/hostedtoolcache"` |
| `runnerDebug` | Режим отладки | `"1"` (если включен) |
...
- name: Deployment Notification
  ...
  with:
    ...
    message: |
      ...
      **Окружение:** {{deploymentEnvironment}}
      **Состояние:** {{deploymentState}}
      **Описание:** {{deploymentDescription}}
      **URL:** {{deploymentEnvironmentUrl}}
      **Создатель:** {{deploymentCreator}}
```
...
6. **Расширенный контекст** - больше переменных из GitHub и Runner
...
- ✅ **Runner информация** - детальная информация о среде выполнения
