...
describe('Telegram Notify Action - Basic Tests', () => {
  ...
  test('Template processing works correctly', () => {
    ...
    process.env.MESSAGE = 'Custom message';
    ...
    try {
      ...
      const processedMessage = notify.processTemplate();
      expect(processedMessage).toContain('✅');
      expect(processedMessage).toContain('Success');
      expect(processedMessage).toContain('test/repo');
      expect(processedMessage).toContain('testuser');
      expect(processedMessage).toContain('Custom message');
  ...
  test('Input validation works correctly', () => {
    ...
    try {
      ...
      // Test valid inputs
      process.env = { TELEGRAM_TOKEN: 'token', CHAT_ID: '123', MESSAGE: 'test' };
      notify = new TelegramNotify();
